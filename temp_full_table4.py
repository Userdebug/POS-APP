...
for day in days:
    cursor.execute(
        """
        SELECT t.jour, c.nom as categorie, t.si, t.achats, t.ca, t.sf, t.env, t.vente_theorique, t.marge, t.cloturee
        FROM Tcollecte t
...
# Afficher spécifiquement la catégorie ID=4 (BA)
print("\n=== Détail catégorie ID=4 (BA) ===")
cursor.execute("""
    SELECT t.jour, t.si, t.achats, t.ca, t.sf, t.env, t.vente_theorique, t.marge, t.cloturee
    FROM Tcollecte t
    WHERE t.categorie_id = 4
    ORDER BY t.jour DESC
""")
ba_rows = cursor.fetchall()
print(
    f"{'Jour':<12} {'SI':>12} {'Achats':>10} {'CA':>12} {'SF':>12} {'ENV':>8} {'VenteThéo':>12} {'Marge':>12} {'Clot':>4}"
)
print("-" * 100)
for row in ba_rows:
    r = dict(row)
    print(
        f"{r.get('jour', ''):<12} {r.get('si', 0):>12,} {r.get('achats', 0):>10,} {r.get('ca', 0):>12,} {r.get('sf', 0):>12,} {r.get('env', 0):>8,} {r.get('vente_theorique', 0):>12,} {r.get('marge', 0):>12,} {r.get('cloturee', 0):>4}"
    )
